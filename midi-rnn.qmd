...
"Copyright © " * "2024 - " * Dates.format(d, Dates.DateFormat("yyyy")) * " hiiro" * "\n" |> println
"This notebook is licensed under MIT License." * "\n" |> println
"Last Updated: " * ( d |> string ) |> println
```
...
```{julia}
_input_midi = "data/input.mid"
```

```{julia}
_input_midi isa String || error("Not supported type: $( typeof(_input_midi) )")

isfile(_input_midi) || download(
...
Plots.histogram(
    [ ni.pitch % 12 for ni ∈ _notes ],
    bins = range(0, 11; step = 1)
)
```
...
_tpq, _qpm, relative_notes = JLD2.load("data/input.jld2", "tpq", "qpm", "notes")

relative_notes
```

```{julia}
function classify_notes(notes)
    return  [ [ ni[i] for ni ∈ notes ] for i ∈ 1:4 ] .|> Set .|> collect .|> sort # .|> summary
end
```

```{julia}
@show uniq_pitch, uniq_step, uniq_position, uniq_duration = classify_notes(relative_notes)
```

```{julia}
@assert (filter(uniq_step .% (_tpq ÷ 8)) do v
    v != 0
end |> length) == 0
```
...
```{julia}
new_relative_notes = generate_notes(init; tpq = _tpq, qpm = _qpm, generate_seconds = 600)
```
...
_note_vector = [
    MIDI.Note(ri[1], ( ri[2] + sum(new_relative_notes[1:(i - 1)] .|> v -> v[2]) + (new_relative_notes[begin][3] * (_tpq ÷ 8)) ); duration = ri[4])
...
function save_midi_file(filename, note_vector; tpq = 960, qpm = 120, humanize = true, chunk_size = 2048)
    ...
    MIDI.addtrackname!(_tempo_track, "Tempo")
    ...
    MIDI.addtrackname!(_notes_track, "Piano")

    _instrument_event = MIDI.InstrumentNameEvent(0, 0x04, "piano")
    MIDI.addevent!(_notes_track, 0, _instrument_event)
...
save_midi_file(_sample_file_name, _note_vector; tpq = _tpq, qpm = _qpm)
```
